    ...
    try:
        totalProductsOutput = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.CSS_SELECTOR, "span[automation-id='totalProductsOutputText']"))
        ).text
        pattern = re.compile('Showing.*of\s(\d*)')
        result = int(pattern.findall(totalProductsOutput).pop())
    except:
        return False
    return result
...
def get_sku_items(driver, link, index):
    ...
    for i in range(index):
        ...
        # for each page index, get sku/itemid items in sku/itemid list
        try:
            items = WebDriverWait(driver, 10).until(
                EC.presence_of_all_elements_located(
                    (By.XPATH, "//div[class='product-list grid']"))
            )
            print("yes")
            for item_element in items:
                item = dict()
                item_sku = item_element.get_attribute("itemid")
                print(item_sku)
